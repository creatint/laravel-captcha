...
class Captcha
{
    ...
    )
    {
        ...
        $this->hasher = $hasher;
        $this->str = $str;
        $this->characters = Config::get('captcha.characters', ['1', '2', '3', '4', '6', '7', '8', '9']);
    }
     ...
     * @param string $config
     * @param boolean $api
     * @return array|ImageManager->response
     * @throws \Exception
     */
    public function create_image($config = 'default', $api = false)
    {
    ...
    public function create($config = 'default', $api = false)
    {
        if (!$api) {
            ...
            return $this->create_image($config);
        }

        $data = $this->create_image($config, $api);
        Cache::add($data['key'], true, 300);
        return $data;
    }
